...
add_action("wp_enqueue_scripts", "custom_script_function");
function custom_script_function()
{
	wp_enqueue_script("mamontov-observer-script", get_template_directory_uri() . '/new-site/assets/js/time_spend_Observer.js', array(), '1.0', false);
}
...
function myajax_data()
{
	...
	wp_enqueue_script('mamontov-metriks-script', get_template_directory_uri() . '/new-site/assets/js/metriki.js', array('jquery'), '1.0', true);
...
function load_posts_scroll_callback()
{
	...
	$search = $_POST['searchParam'] ? $_POST['searchParam'] : "";
...
class Footer_Walker_Nav_Menu extends Walker_Nav_Menu
{
	...
	function start_el(&$output, $data_object, $depth = 0, $args = null, $current_object_id = 0)
	{
		...
		$item_output = sprintf(
			'%1$s<a class="opener__link" data-title="%3$s%4$s%5$s" %2$s>%3$s%4$s%5$s</a></span>%6$s',
			$args->before,
			$attributes,
			$args->link_before,
...
class Header_new_Walker_Nav_Menu extends Walker_Nav_Menu
{
	...
	function start_lvl(&$output, $depth = 0, $args = NULL)
	{
		...
		if ($depth === 1) {
			$output .= '<div class="wrap_sub_menu"><ul class="sub">';
		}
	}
...
// Скрыть страницу Черновик от пользователей
add_action('template_redirect', function () {
	if (is_page() || is_single()) {
		global $post;
		if ($post->post_status !== 'publish' && !current_user_can('edit_posts')) {
			status_header(404);        // выставляем код ответа
			nocache_headers();         // запрещаем кэш
			include get_404_template(); // подключаем стандартный шаблон 404.php
			exit;
		}
	}
});
